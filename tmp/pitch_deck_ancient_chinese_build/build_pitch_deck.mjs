...
const buildDir = process.env.TMP_DIR;
const runtimePython = process.env.RUNTIME_PYTHON;
if (!out || !skillDir || !buildDir || !runtimePython) throw new Error("Missing required presentation runtime paths");
...
function text(slide, value, x, y, w, h, size = 20, color = C.cream, opts = {}) {
  ...
  s.text = value;
  s.text.style = { typeface: fontFamily, fontSize: size, color, bold: !!opts.bold, alignment: opts.align || "left", verticalAlignment: opts.valign || "middle" };
  return s;
}
...
async function base(slide, index, section = "MAHJONG × BLOCK") {
  await image(slide, `${figma}/01_menu_gate_closed.png`, 0, 0, W, H, "cover", { left: 0, top: 0, right: 0, bottom: 0.75 });
  slide.shapes.add({ geometry: "rect", position: { left: 0, top: 0, width: W, height: H }, fill: "#1B1009/76", line: { style: "solid", fill: "none", width: 0 } });
  await image(slide, `${figma}/05_gameplay.png`, 0, 18, 58, 654, "cover", { left: 0, top: 0, right: 0.87, bottom: 0 });
  await image(slide, `${figma}/05_gameplay.png`, 1222, 18, 58, 654, "cover", { left: 0.87, top: 0, right: 0, bottom: 0 });
  ...
  text(slide, section, 72, 34, 360, 22, 16, C.gold, { bold: true });
  text(slide, String(index).padStart(2, "0"), 1140, 34, 68, 22, 16, C.muted, { bold: true, align: "right" });
  rule(slide, 72, 672, 1136, C.line, 1);
  text(slide, "GDD Tổng Hợp — Mahjong × Block (Final Core)", 72, 682, 620, 18, 12, C.muted);
}
...
async function image(slide, file, x, y, w, h, fit = "cover", crop) {
  slide.images.add({ blob: await bytes(file), contentType: "image/png", alt: "Product UI screenshot", fit, position: { left: x, top: y, width: w, height: h }, geometry: "roundRect", borderRadius: "rounded-xl", ...(crop ? { crop } : {}) });
}
...
async function main() {
  ...
  // 2 — hook
  {
    const s = p.slides.add(); await base(s, 2, "CORE HOOK"); title(s, "Không chọn cặp có sẵn — người chơi tạo ra cơ hội ghép", "Mỗi lượt là một quyết định đặt polyomino, không phải thao tác chọn hai quân.");
  ...
  // 3 — rules
  {
    const s = p.slides.add(); await base(s, 3, "CORE RULE"); title(s, "Hai tầng biến “lộ mặt” thành một lựa chọn chiến thuật", "Chỉ quân cùng mặt, cùng tầng, kề cạnh và đang lộ mới Match.");
    const xs = [72, 430, 788];
    const vals = [
  ...
  // 4 — core loop
  {
    const s = p.slides.add(); await base(s, 4, "CORE LOOP"); title(s, "Core loop tập trung vào đọc bàn, lựa chọn và phản ứng dây chuyền", "Một lượt có thể tạo nhiều wave Match liên tiếp.");
  ...
  // 6 — difficulty
  {
    const s = p.slides.add(); await base(s, 6, "CONTENT"); title(s, "Độ khó tăng bằng không gian, áp lực và điều kiện mở khóa", "Không cần thêm luật Match mới để tạo biến thể màn chơi.");
  ...
  // 7 — boosters
  {
    const s = p.slides.add(); await base(s, 7, "SUPPORT"); title(s, "Booster hỗ trợ quyết định, không thay đổi điều kiện thắng", "Đổi khối và Hint cùng dùng kho chung, nhưng Level 1 cố ý khóa chúng để dạy luật cơ bản.");
  ...
  // 8 — retention
  {
    const s = p.slides.add(); await base(s, 8, "ENGAGEMENT"); title(s, "Vòng lặp hằng ngày đặt trên core loop thay vì thay thế nó", "Quest và điểm danh tái sử dụng Xu / booster, không có kho tài nguyên riêng.");
  ...
  // 9 — economy
  {
    const s = p.slides.add(); await base(s, 9, "ECONOMY"); title(s, "Economy thiên về cosmetic, với booster là một sink cần được cân nhắc", "GDD quy định economy không được chỉnh luật thắng / thua của core loop.");
  ...
  // 10 — readiness
  {
    const s = p.slides.add(); await base(s, 10, "READINESS"); title(s, "Nội dung deterministic đã có “lan can” kỹ thuật; rủi ro chính còn là trải nghiệm thật", "Mỗi lần tải, self-test kiểm tra luật, dữ liệu và lời giải authored.");
    const cards = [
